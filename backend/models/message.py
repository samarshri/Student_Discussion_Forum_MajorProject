...
class Message:
    ...
    def __init__(self, id=None, conversation_id=None, sender_id=None, content=None,
                 ...
                 created_at=None, edited_at=None, sender=None, timestamp=None):
        self.id = id
        ...
        # Handle both created_at (new) and timestamp (legacy)
    ...
    def from_dict(data):
        ...
        for field in ['created_at', 'timestamp', 'edited_at']:
            val = data.get(field)
            if isinstance(val, str):
                try:
                    data[field] = datetime.strptime(val, '%Y-%m-%d %H:%M:%S.%f')
                except ValueError:
                    try:
                        data[field] = datetime.strptime(val, '%Y-%m-%d %H:%M:%S')
                    except ValueError:
                        pass
